...
class MetaLoader:
    ...
    def __init__(
        ...
        self.names: List[str] = []
        ratios: List[int] = []
        self.sampling_pools = torch.tensor([])
        ...
        self.device = device
        self.sampling_pools = torch.tensor(ratios).float().to(self.device)
        self.distributed = distributed
        self.step = 0

    def __iter__(self) -> Iterator[Tuple]:
        """this iterator will run_r2r_il.sh indefinitely"""
        task_id = self.sampling_pools[0]
        epoch_id = 0
        while True:
            if self.step % self.accum_steps == 0:
                task_id = torch.multinomial(self.sampling_pools, 1)
                if self.distributed:
                    # make sure all process is training same task
                    dist.broadcast(task_id, 0)
            self.step += 1
            task = self.names[int(task_id.cpu())]
...
def build_dataloader(task, dataset, collate_fn, is_train: bool, opts):
    ...
    if opts.local_rank == -1:
        ...
        if size > 1:
            if is_default_gpu(opts):
                LOGGER.info(
                    f"DataParallel: The batch size is set to {batch_size} * {size}"
                )
            batch_size *= size
