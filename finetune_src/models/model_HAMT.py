...
class VLNBertCMT(nn.Module):
    def __init__(self, args):
        ...
        self.vis_config, self.vln_bert = get_vlnbert_models(args, config=None)  # initialize the VLN-BERT
    ...
    def forward(self, mode, txt_ids=None, txt_masks=None, txt_embeds=None, 
                ...
                ob_embeds=None, ob_masks=None, return_states=False):
        ...
        if mode == 'language':
            ...
            # only O_t
            ob_img_feats = self.drop_env(ob_img_feats)
            ob_embeds = self.vln_bert(
                mode, ob_img_feats=ob_img_feats, ob_ang_feats=ob_ang_feats,
                ob_nav_types=ob_nav_types, ob_masks=ob_masks)
            return ob_embeds
            ...
            txt_embeds, hist_embeds, ob_embeds = self.vln_bert(
                mode, txt_embeds=txt_embeds, txt_masks=txt_masks,
                hist_embeds=hist_embeds, hist_masks=hist_masks,
                ob_embeds=ob_embeds, ob_masks=ob_masks)
            return txt_embeds, hist_embeds, ob_embeds
